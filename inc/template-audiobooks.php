...
$query = 'select distinct b.id, b.product_image_id, b.title, f.post_title as pen_name, b.book_description, b.createdate, g.id as product_url
        ...
        limit 48'; //48
$results = $wpdb->get_results($wpdb->prepare($query, $books_table, $book_genres_table, $genres_table, $book_products_table, $product_types_table, $pen_names_table, $posts_table, $posts_table, 'audiobooks'), ARRAY_A);
...
?><main class="full-screen">
    <div class="banner"><h1 class="centered-text banner-heading-46">Audiobooks By Our Authors</h1></div>
    <div>
        <?php if ($results) {
        ...
        <?php } else {
            ?><p class="centered-text padded-paragraph-20-x">No audiobooks have been added yet. Check back soon!</p>
        <?php } ?>
    </div>
</main>
